...
void Inventory::render(void)
{
	for (int i = 0; i < _im->getItem().size(); i++)
	{
			...
			{
				{
					FONTMANAGER->drawText(getMemDC(), 275, 170,
						"µÕ±Ù¸ð²Ã", 25, 100, _im->getItem()[i]->getName(), _im->getItem()[i]->getName().size(), BTEXT);

					FONTMANAGER->drawText(getMemDC(), 195, 225,
						"µÕ±Ù¸ð²Ã", 22, 100, _im->getItem()[i]->getScript(),
					...
					if (_im->getItem()[i]->getEquip())
					{
						FONTMANAGER->drawText(getMemDC(), 340 + 70, 360,
							"µÕ±Ù¸ð²Ã", 25, 100, _InvenTEXT[1].text, wcslen(_InvenTEXT[1].text), BTEXT);
					}
					else
					{
						FONTMANAGER->drawText(getMemDC(), 340 + 70, 360,
							"µÕ±Ù¸ð²Ã", 25, 100, _InvenTEXT[0].text, wcslen(_InvenTEXT[0].text), BTEXT);
					}
					FONTMANAGER->drawText(getMemDC(), 495 + 70, 360,
						"µÕ±Ù¸ð²Ã", 25, 100, _InvenTEXT[2].text, wcslen(_InvenTEXT[2].text), BTEXT);
				}
			}

			FONTMANAGER->drawText(getMemDC(), 275, 170,
				"µÕ±Ù¸ð²Ã", 25, 100, rosary[_selection].name, wcslen(rosary[_selection].name), BTEXT);

			FONTMANAGER->drawText(getMemDC(), 195, 225,	"µÕ±Ù¸ð²Ã", 22, 100, rosary[_selection].script,
				((20 > wcslen(rosary[_selection].script)) ? wcslen(rosary[_selection].script) :20),	MTEXT);

			if (wcslen(rosary[_selection].script) > 20)
			{
				FONTMANAGER->drawText(getMemDC(), 195, 255, "µÕ±Ù¸ð²Ã", 22, 100, rosary[_selection].script + 20,
		...
		case 3:
			...
			{
				FONTMANAGER->drawText(getMemDC(), 195, 255, "µÕ±Ù¸ð²Ã", 22, 100, quest[_selection].script + 20,
		...
		case 4:
			...
			if (wcslen(quest[_selection].script) > 20)
			{
				FONTMANAGER->drawText(getMemDC(), 195, 255, "µÕ±Ù¸ð²Ã", 22, 100, meaculpa[_selection].script + 20,
		...
		case 5:
			...
			if (wcslen(quest[_selection].script) > 20)
			{
				FONTMANAGER->drawText(getMemDC(), 195, 255, "µÕ±Ù¸ð²Ã", 22, 100, bible[_selection].script + 20,
		...
		case 7:
			...
			{
				FONTMANAGER->drawText(getMemDC(), 195, 255, "µÕ±Ù¸ð²Ã", 22, 100, collection[_selection].script + 20,
